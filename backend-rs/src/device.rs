...
pub fn list_ble_devices() -> BleDevicesResponse {
    BleDevicesResponse { devices: Vec::new() }
}
...
pub fn send_to_serial(request: SerialSendRequest) -> Result<SendResponse, DeviceError> {
    ...
    if request.pixel_matrix.is_empty()
        || request.pixel_matrix.first().map(|row| row.is_empty()).unwrap_or(true)
    {
    ...
    let grid_size = [
        request.pixel_matrix.first().map(|row| row.len()).unwrap_or(0),
...
pub fn send_to_ble_backend() -> BleSendResponse {
    BleSendResponse {
        success: false,
        message: "Rust backend BLE API is unavailable; use Web Bluetooth in the browser".to_string(),
        bytes_sent: 0,
        duration_ms: 0,
    }
}
...
fn parse_led_matrix_size(raw: &str) -> (usize, usize) {
    ...
    let width = parts.next().and_then(|part| part.parse::<usize>().ok()).unwrap_or(64);
    let height = parts.next().and_then(|part| part.parse::<usize>().ok()).unwrap_or(64);
    (width, height)
}

fn build_packet(pixel_matrix: &[Vec<Option<String>>], background_color: [u8; 3], led_size: (usize, usize)) -> Vec<u8> {
...
fn pixel_matrix_to_rgb565(pixel_matrix: &[Vec<Option<String>>], background_color: [u8; 3]) -> Vec<u8> {
    let background = background_fill_rgb565(background_color);
    let palette = global_palette();
    let mut data = Vec::with_capacity(pixel_matrix.len() * pixel_matrix.first().map(|row| row.len()).unwrap_or(0) * 2);
...
fn scale_and_center_image(
    ...
    for led_y in 0..led_height {
        for led_x in 0..led_width {
            ...
            if rel_x < 0 || rel_y < 0 || rel_x as usize >= scaled_width || rel_y as usize >= scaled_height {
                continue;
            }
...
fn background_fill_rgb565(background_color: [u8; 3]) -> u16 {
    let rgb565 = rgb_to_rgb565(background_color[0], background_color[1], background_color[2]);
