...
fn render_pdf_pattern_page(request: &ExportRequest) -> Result<RgbImage, ExportError> {
    ...
    let grid_w = request
        ...
        .map(|row| row.len())
        .unwrap_or(0) as u32;
    let coord_px = if request.show_coordinates {
        mm_to_px(8.0, 150.0)
    } else {
        0
    };
...
fn render_pattern_image(request: &ExportRequest) -> Result<RgbImage, ExportError> {
    ...
    let summary_rows =
        ((request.color_summary.len() as u32 + colors_per_row - 1) / colors_per_row).max(1);
            ...
            match request.pixel_matrix[y][x].as_deref() {
                ...
                Some(code) => {
                    ...
                    if show_codes_in_cells && cell_size >= 16 {
                        let text_color = if luminance(fill) > 128 {
                            rgb(0, 0, 0)
                        } else {
                            rgb(255, 255, 255)
                        };
