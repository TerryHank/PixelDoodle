...
use image::{
    ...
    ColorType, DynamicImage, ImageEncoder, ImageFormat, ImageReader, Rgb, RgbImage,
};
use rayon::prelude::*;
use thiserror::Error;
...
pub fn process_image_bytes(bytes: &[u8], options: GenerateOptions) -> Result<EngineOutput, EngineError> {
    let rgb = decode_rgb_image(bytes)?;
    let palette = global_palette();
    process_rgb_image(rgb, palette, &options)
}

pub fn process_dynamic_image(image: DynamicImage, options: GenerateOptions) -> Result<EngineOutput, EngineError> {
    let palette = global_palette();
...
pub fn debug_subpalette_codes(
    ...
    Ok(debug_pipeline(bytes, options)?
        .subpalette_codes)
}
...
fn debug_pipeline_inner(
    ...
    let palette = global_palette();
    let (contrasted, mean_gray, p5, p95, contrast_factor) = apply_contrast_debug(&image, options.contrast);
    ...
    if options.similarity_threshold > 0 {
        merge_similar_colors(&mut final_matrix_indices, palette, options.similarity_threshold as f64);
    }
...
fn process_rgb_image(
    ...
    image = preprocess_image(image, options.contrast, options.saturation, options.sharpness);
    image = consolidate_extremes(image);
    ...
    let pixel_matrix = render_pixel_matrix(&matrix, palette);
    let color_summary = build_color_summary(&pixel_matrix, palette);
    let total_beads = color_summary.iter().map(|entry| entry.count).sum();
    let preview_image = generate_preview_base64(&pixel_matrix, palette, 400)?;
...
fn decode_rgb_image(bytes: &[u8]) -> Result<RgbImage, EngineError> {
    match image::guess_format(bytes)? {
        ImageFormat::Jpeg => decode_jpeg_with_mozjpeg(bytes),
        _ => Ok(ImageReader::new(std::io::Cursor::new(bytes))
            .with_guessed_format()?
            .decode()?
            .to_rgb8()),
    }
...
fn decode_jpeg_with_mozjpeg(bytes: &[u8]) -> Result<RgbImage, EngineError> {
    let dec = mozjpeg::Decompress::with_markers(mozjpeg::NO_MARKERS)
        .from_mem(bytes)?;
    let (width, height) = dec.size();
    let mut dec = dec.rgb()?;
    let pixels: Vec<[u8; 3]> = dec.read_scanlines()?;
    let raw = pixels
        .into_iter()
        .flat_map(|pixel| pixel.into_iter())
        .collect::<Vec<_>>();
    RgbImage::from_raw(width as u32, height as u32, raw).ok_or_else(|| {
        std::io::Error::new(std::io::ErrorKind::InvalidData, "mozjpeg decoded buffer size mismatch").into()
    })
}
...
fn consolidate_extremes(image: RgbImage) -> RgbImage {
    ...
    RgbImage::from_fn(image.width(), image.height(), |x, y| {
        let pixel = image.get_pixel(x, y);
        let mut rgb = [f64::from(pixel[0]), f64::from(pixel[1]), f64::from(pixel[2])];
...
fn select_top_n_colors(
    ...
    let mapped = pixels
        .par_iter()
        .map(|pixel| closest_palette_global(palette, *pixel, allowed_indices))
        .collect::<Vec<_>>();
    ...
    let mut selected = ranked.into_iter().take(n).map(|(idx, _)| idx).collect::<Vec<_>>();
    selected.sort_unstable();
    selected
}

fn quantize_to_subpalette(image: &RgbImage, palette: &ArtkalPalette, sub_palette: &[usize]) -> Vec<usize> {
    image
        .as_raw()
        .par_chunks_exact(3)
        .map(|pixel| closest_subpalette_local(palette, [pixel[0], pixel[1], pixel[2]], sub_palette))
        .collect()
}

fn dither_to_subpalette(image: &RgbImage, palette: &ArtkalPalette, sub_palette: &[usize]) -> Vec<usize> {
    ...
    for y in 0..height {
        for x in 0..width {
            ...
            let local = closest_subpalette_local(palette, old_pixel, sub_palette);
            result[idx] = local;
...
fn diffuse_error(buffer: &mut [[f64; 3]], width: usize, height: usize, x: usize, y: usize, error: [f64; 3]) {
...
fn cleanup_rare_colors(matrix: &mut [Vec<Option<usize>>], palette: &ArtkalPalette, total_pixels: usize, min_ratio: f64) {
    ...
    let kept = freq
        .iter()
        .filter_map(|(code, count)| if *count >= min_count { Some(*code) } else { None })
        .collect::<Vec<_>>();
    if kept.is_empty() {
        return;
    }
...
fn merge_similar_colors(matrix: &mut [Vec<Option<usize>>], palette: &ArtkalPalette, threshold: f64) {
    let mut freq = HashMap::<usize, usize>::new();
    for row in matrix.iter() {
        for code in row.iter().flatten() {
            *freq.entry(*code).or_insert(0) += 1;
...
fn cap_max_colors(matrix: &mut [Vec<Option<usize>>], palette: &ArtkalPalette, max_colors: usize) {
    ...
    let mut ranked = freq.into_iter().collect::<Vec<_>>();
    ranked.sort_by(|lhs, rhs| rhs.1.cmp(&lhs.1).then_with(|| lhs.0.cmp(&rhs.0)));
    let kept = ranked.iter().take(max_colors).map(|(code, _)| *code).collect::<Vec<_>>();
    let mut remap = HashMap::<usize, usize>::new();
...
fn render_pixel_matrix(matrix: &[Vec<Option<usize>>], palette: &ArtkalPalette) -> Vec<Vec<Option<String>>> {
    matrix
        .iter()
        .map(|row| {
            row.iter()
                .map(|cell| cell.and_then(|index| palette.get_color(index).map(|color| color.code.clone())))
                .collect::<Vec<_>>()
        })
        .collect()
}

fn build_color_summary(pixel_matrix: &[Vec<Option<String>>], palette: &ArtkalPalette) -> Vec<ColorSummaryEntry> {
...
fn generate_preview_base64(
    ...
    let cell_size = (max_size / width.max(height)).clamp(1, 10);
    let mut image = RgbImage::new((width * cell_size) as u32, (height * cell_size) as u32);

    for (y, row) in pixel_matrix.iter().enumerate() {
        for (x, code) in row.iter().enumerate() {
            let rgb = code
                .as_ref()
                .and_then(|value| palette.get_by_code(value))
                .map(|color| color.rgb)
                .unwrap_or([220, 220, 220]);
            for dy in 0..cell_size {
                for dx in 0..cell_size {
                    image.put_pixel((x * cell_size + dx) as u32, (y * cell_size + dy) as u32, Rgb(rgb));
                }
            }
        }
    }
            ...
            for y in 0..other_size {
                for xx in 0..out_size {
                    ...
                    out.put_pixel(
                        xx as u32,
                        y as u32,
                        Rgb([clip8_precision(sums[0]), clip8_precision(sums[1]), clip8_precision(sums[2])]),
                    );
                }
            }
        }
...
fn precompute_lanczos_coeffs(in_size: usize, out_size: usize) -> (Vec<(usize, usize)>, Vec<i32>, usize) {
    ...
    for xx in 0..out_size {
        ...
        if weight_sum != 0.0 {
            for x in 0..xmax_count {
                let weight = lanczos_filter(((x + xmin_usize) as f64 - center + 0.5) * ss) / weight_sum;
                coeff_slice[x] = weight_to_precision(weight);
            }
        }
