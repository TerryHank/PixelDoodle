...
fn select_top_n_colors(
    ...
    let mapped = pixels
        .par_iter()
        .map_init(
            || GlobalPaletteMatcher::new(palette, allowed_indices),
            |matcher, pixel| matcher.closest(*pixel),
        )
        .collect::<Vec<_>>();
...
fn quantize_to_subpalette(
    ...
    image
        ...
        .par_chunks_exact(3)
        .map_init(
            || SubPaletteMatcher::new(palette, sub_palette),
            |matcher, pixel| matcher.closest([pixel[0], pixel[1], pixel[2]]),
        )
        .collect()
}
