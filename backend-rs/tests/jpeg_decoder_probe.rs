...
fn python_executable() -> String {
    env::var("PYTHON_EXECUTABLE").unwrap_or_else(|_| {
        "D:\\Workspace\\PixelDoodle-web\\.venv\\Scripts\\python.exe".to_string()
    })
}
...
#[test]
fn current_jpeg_decoder_probe_runs_against_pillow() {
    ...
    assert_ne!(pillow_crc, 0);
    assert_ne!(image_rs_crc, 0);
    println!("pillow_crc={pillow_crc} image_rs_crc={image_rs_crc}");
}
