...
#[tokio::test]
async fn generate_route_keeps_preview_field_as_empty_placeholder() {
    let root = repo_root();
    let image_path = root
        .join("docs")
        .join("examples")
    ...
    let boundary = "beadcraft-runtime-boundary";
    ...
    assert!(payload.get("preview_image").is_some());
    assert_eq!(payload["preview_image"], Value::String(String::new()));
}
