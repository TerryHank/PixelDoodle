...
impl ArtkalPalette {
    pub fn load() -> Self {
        let colors: Vec<PaletteColor> =
            serde_json::from_str(include_str!("../../data/artkal_m_series.json"))
                .expect("palette json should parse");
        let presets: HashMap<String, PalettePreset> =
            serde_json::from_str(include_str!("../../data/artkal_presets.json"))
