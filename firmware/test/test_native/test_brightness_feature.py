...
class BrightnessFeatureTests(unittest.TestCase):
    ...
    def test_backend_exposes_wifi_brightness_routes(self):
        routes = {
            (route.path, method)
        ...
        self.assertIn(("/api/wifi/brightness", "GET"), routes)
        self.assertIn(("/api/wifi/brightness", "POST"), routes)
        self.assertIn(("/wifi/brightness", "GET"), routes)
        self.assertIn(("/wifi/brightness", "POST"), routes)

    def test_backend_source_relays_wifi_brightness_to_device(self):
        content = MAIN_PY.read_text(encoding="utf-8")
        self.assertIn('f"http://{entry[\'ip\']}:8766/brightness"', content)
        self.assertIn("requests.get(", content)
    ...
    def test_ble_receiver_supports_brightness_protocol_and_wifi_endpoint(self):
        ...
        required_patterns = [
            ...
            r"if \(requestLine\.startsWith\(\"GET /brightness\"\)\)",
            r"if \(requestLine\.startsWith\(\"POST /brightness\"\)\)",
