...
基线；v13 完成“创物集 DIY 拼豆”视觉升级、移动端校准与应用图标；v14.0.4 将应用名统一为
“DIY拼豆”，区分像素画图纸导入与照片量化导入，完成选图、网格校准、裁剪、本地识别、固定画框结果预览、色号校对和进入编辑的完整流程；豆子数量直接决定分辨率，预览画框尺寸不随钉板变化，图案按比例居中并自动留白，不拉伸、不溢出。同时保留配色方案/颜色套餐/六种钉板常驻设置、底栏激活着色、素材移动端双列铺满预览，以及 Android WebView 无法 `fetch(blob:)` 时的离线生成修复。V14 内置
1,200 条离线素材，并按本期要求移除万相/通义生成入口与运行时初始化；相关适配器源码保留，
后续需要时可重新接回。
...
- 本地图片像素量化并自动转换为拼豆图（V14 不调用万相/通义云生成）
